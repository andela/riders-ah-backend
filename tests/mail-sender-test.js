...
const testHtml = `<html>Please click on the following <strong><a href="/api/v1/auth/verify/${Helper.tokenGenerator(
	20
)}"> link</a></strong> to verify your email</html>`;
...
describe('Function for sending email', () => {
	...
	it('No email provided', (done) => {
		mailSender.send(noEmail).then().catch((error) => {
	...
	it('Provided wrong email', (done) => {
		mailSender.send(wrongEmail).then().catch((error) => {
	...
	it('No subject provided', (done) => {
		mailSender.send(noSubject).then().catch((error) => {
	...
	it('No html content provided', (done) => {
		mailSender.send(noHtml).then().catch((error) => {
